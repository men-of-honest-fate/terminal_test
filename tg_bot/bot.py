...
class Bot:
    def __init__(self):
        ...
        def start_message(message):
            ...
            self.bot.set_state(
                message.from_user.id, MyStates.wait_for_button, message.chat.id
            )
        ...
        def authorisation_start(message):
            if message.text == "Авторизоваться":
                ...
                self.bot.set_state(
                    message.from_user.id, MyStates.wait_for_login, message.chat.id
                )
        ...
        def login_hashing(message):
            ...
            self.bot.send_message(
                message.chat.id, "Логин принят", reply_markup=self.markup
            )
            self.bot.set_state(
                message.from_user.id, MyStates.wait_for_password, message.chat.id
            )
            self.bot.send_message(
        ...
        def password_hashing(message):
            ...
            self.bot.send_message(
                message.chat.id, "Пароль принят", reply_markup=self.markup
            )
            response = requests.post(
                f"{self.api_url}/authorize",
                data={
                    "input_login": self.login_hash,
                    "input_password": self.password_hash,
                },
            )
            if response.status_code == 200:
                self.bot.send_message(
                    message.chat.id,
                    f"Данные приняты, Ваш токен сессии {response.request}",
                    reply_markup=self.markup,
                )
            else:
                self.bot.send_message(
                    message.chat.id, "Вы незарегестрированы", reply_markup=self.markup
                )
            self.bot.set_state(
                message.from_user.id, MyStates.wait_for_changing, message.chat.id
            )
        ...
        def changing_data(message):
            self.bot.send_message(
                message.chat.id,
                "Хотите сменить логин или пароль?",
                reply_markup=self.markup,
            )
            self.bot.send_message(
                message.chat.id,
                "Тогда введите токен одной из прошлых сессий",
                reply_markup=self.markup,
            )
            response = requests.post(
                f"{self.api_url}/check_token", data={"token": message}
            )
            if response.status_code == 200:
                self.bot.set_state(
                    message.chat.id,
                    MyStates.wait_for_changing_login_or_password,
                    message.chat.id,
                )
            else:
                self.bot.send_message(
                    message.chat.id, "Ваш токен неверен", message.chat.id
                )
        ...
        def button_message(message):
            if message.text == "Оплатить":
                ...
                self.bot.set_state(
                    message.from_user.id, MyStates.wait_for_summ, message.chat.id
                )
                self.bot.send_message(
                    message.chat.id,
                    "Для оплаты введите сумму",
                    reply_markup=self.markup,
                )
            elif message.text == "Отмена":
                self.bot.send_message(
                    message.chat.id,
                    "Ваша оплата была отменена. \nДля того чтобы оплатить снова нажмите кнопку оплатить",
                    reply_markup=self.markup,
                )
                self.bot.set_state(
                    message.from_user.id, MyStates.wait_for_button, message.chat.id
        ...
        def cancel_payment(message):
            if message.text.isdigit():
                self.bot.send_message(
                    message.chat.id,
                    "Ваша оплата была принята в обработку. Скоро мы отправим Вам QR код СБП",
                )
                try:
                    response = requests.post(
                        f"http://{self.api_url}/purchase/{int(message.text)}"
                    )
                    self.bot.send_message(message.chat.id, response.text)
                except Exception as e:
                    print(e)
                    self.bot.send_message(
                        message.chat.id, "Сервер недоступен, повторите попытку позже"
                    )
            else:
                if message.text == "Отмена":
                    ...
                    self.bot.send_message(
                        message.chat.id,
                        "Введена неверная сумма для оплаты. Повторите попытку",
                    )
